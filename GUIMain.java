			...
			if (counter % 2 == 0) {
				...
				if (StdDraw.isMousePressed()) {
					...
					if (check == true) {
						...
						// These two lines tested if the code worked
//						StdDraw.text(.5, .1, "Counter: " + counter);
//						StdDraw.show();
					}
			...
			if (counter % 2 == 1) {
				...
				if (StdDraw.isMousePressed()) {
					...
					boolean check = false;
					for (int i = 0; i < moves.size(); i++) {
						if (moves.get(i).equalsIgnoreCase(position)) {
							check = true;
							moves.remove(i); // Basically this move is no longer possible to make
						}
					}
		...
		// Check the edge case again
		if (play.hasPlayerWon() == false && counter == 8) {
			boolean mouse = false;
			if (StdDraw.isMousePressed()) {
				mouse = true;
				String move = "";
				...
				if (check == true) {
					...
					StdDraw.text(.5, .3, "Invalid move, game terminated.");
				}
			}
		}
		...
		if (play.hasPlayerWon() == false && counter == 9) {
			StdDraw.text(.5, .3, "Tie Game!");
		}
	}
